...
def get_inspiral_snr(freq, m1, m2, z, cosmo, psd_func, ndet=1, tobs=3):
    nz = len(z)
    ...
    return np.sqrt(ndet * factor * integrated_quantity)


def get_snr_of_sgwb(freq, omega_gw, psd, cosmo, duration=3, orf=1.0, ndet=1):
    ...
    return (factor * np.sqrt(integrated_quantity)).decompose() * np.sqrt(math.comb(ndet, 2))
...
def get_Omega_error_with_frequncy_dependent_zrange(farray, m1, m2, merger_rate_density_func, cosmo, zmin, zmax, psd, Np=11, ndet=1, tobs=3, dz=0.001, projection=False):
    ...
    snrsqlist = get_inspiral_snr(farray, m1, m2, zsample, cosmo, psd, ndet, tobs)**2
    if projection:
